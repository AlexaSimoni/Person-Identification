...
class FlowTracker:
    ...
    def update_bbox_with_optical_flow(self, prev_frame: np.ndarray, next_frame: np.ndarray, bbox: tuple,
       ...
       # scale_factor = np.clip(scale_factor, 0.9, 1.5)
        ...
        #new_w = int(w / scale_x * scale_factor)
        #new_h = int(h / scale_y * scale_factor)
        ...
        # Min/max size and clamp to frame

      #  new_w = max(min_w, min(new_w, w_frame))
      #  new_h = max(min_h, min(new_h, h_frame))

        #new_w = min(new_w, w_frame - x_new)
        #new_h = min(new_h, h_frame - y_new)
